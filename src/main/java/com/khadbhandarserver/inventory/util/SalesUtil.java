...
public class SalesUtil {
	...
	public UpdatedSalesRecord stockRecordUpdate(List<SalesRecordDto> salesRecordDto, StockDetails stockDetails) {
		...
		int requestedQuantity = salesRecordDto.stream()
				.filter(f -> f.getSoldItemName().equalsIgnoreCase(stockDetails.getStockName()))
				.map(quantity -> quantity.getSoldItemQuantity())
