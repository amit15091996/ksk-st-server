...
public class GoogleDriveUtils {
	...
	private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException {
		...
		LocalServerReceiver receiver = new LocalServerReceiver.Builder().setPort(9098)
				.setCallbackPath("/login/oauth2/code/google").build();
	...
	public Drive getGoogleDriveInstance() throws GeneralSecurityException, IOException {
		...
		log.info("-------access token expiration-------- "
				+ Instant.now().plusMillis(getCredentials(HTTP_TRANSPORT).getExpirationTimeMilliseconds()));
		return service;
	}
